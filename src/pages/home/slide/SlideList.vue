...
import FeedmeCard from '@/components/FeedmeCard.vue'
...
const baseRender = slideItemRender({ ...props.cbs, ...p })
const render = (item, index, play, uniqueId) => {
  if (index === 3) {
    return <FeedmeCard />
  }
  return baseRender(item, index, play, uniqueId)
}
