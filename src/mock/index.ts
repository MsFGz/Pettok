...
const customVideos = [
  {
    url: '/videos/6670944e1824341ec2c3856db5a777f1.mp4',
    desc: 'Claude Code 创始人：如何在 AI 时代取得成功 Claude Code 创始人：如何在 AI 时代取得成功的建议#AI #人工智能 #AI时代 #AI浪潮 3月1日',
    author: 'KrillinAI'
  },
  {
    url: '/videos/9274a088b5522d87a7718ccb11332406.mp4',
    desc: '第 2 集 | 出走百度、OpenAI，归来已是AI新王一达里奥·阿莫代伊 程序员最爱又最恨的男人，带领Anthropic成为世界第一AI公司，年化收入300亿美金，超越OpenAI，Claude新模型也远超ChatGPT。#AI #anthropic #Claude #人工智能 #编程 4月11日',
    author: '赛博学妹科技局'
  },
  {
    url: '/videos/bf59d04f384410579682a416685eb1a5.mp4',
    desc: 'claude 克劳德代码会毁了你的生活。让这成为对所有企业家和有创造力的人的警告。它会带走你所有的时间。#claude #claudecode #ai #ai工具 #新人如何做抖音 5月5日',
    author: 'GitHunter'
  }
];

for (let i = 0; i < 3; i++) {
  if (allRecommendVideos[i]) {
    allRecommendVideos[i].video.play_addr.url_list = [customVideos[i].url]
    allRecommendVideos[i].desc = customVideos[i].desc
    if (allRecommendVideos[i].author) {
      allRecommendVideos[i].author.nickname = customVideos[i].author
    } else {
      allRecommendVideos[i].author = { nickname: customVideos[i].author }
    }
  }
